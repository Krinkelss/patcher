...
namespace ConstexprHashes
{
	...
	{
		static constexpr char make_value( const char* data, size_t length )
		{
	...
	{
		static constexpr char make_value( const char*, size_t )
		{
			return 0x80;
		}
	};
	...
	{
		static constexpr char make_value( const char*, size_t length )
		{
			return length << 3;
		}
	};
	...
	constexpr buffer_type make_buffer_helper( const char* data, size_t length, index_tuple<indexes...> )
	{
		return buffer_type{ { buffer_builder<n - 1, indexes>::make_value( data, length )... } };
	}

	// Creates the actual buffer

	template<size_t n>
	constexpr buffer_type make_buffer( const char* data, size_t length )
	{
		return make_buffer_helper<n>( data, length, index_tuple<64>::type() );
	}
	...
	constexpr md5_type md5( const void* data, size_t length )
	{
		return md5_step<0, 0>::do_step( make_buffer<64>( reinterpret_cast< const char* >( data ), length ).data(), 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476 );
	}
