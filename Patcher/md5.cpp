...
#define GetUi32(p) ( \
             ((const byte *)(p))[0]        | \
    ((UINT)((const byte *)(p))[1] <<  8) | \
    ((UINT)((const byte *)(p))[2] << 16) | \
    ((UINT)((const byte *)(p))[3] << 24))
...
#define LOAD_DATA(i)  GetUi32((const UINT *)(const void *)data + (i))
...
static void Md5_UpdateBlocks( UINT state[ 4 ], const byte *data, size_t numBlocks )
{
	UINT a, b, c, d;
...
void Md5_Final( CMd5 *p, byte *digest )
{
	...
	memset( &p->buffer[ pos ], 0, ( MD5_BLOCK_SIZE - 4 * 2 ) - pos );
	{
		...
		SetUi32( p->buffer + MD5_BLOCK_SIZE - 4 * 2, ( UINT )( numBits ) )
			SetUi32( p->buffer + MD5_BLOCK_SIZE - 4 * 1, ( UINT )( numBits >> 32 ) )
	}
	Md5_UpdateBlock( p );
